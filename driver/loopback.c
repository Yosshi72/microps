#include <stdio.h>
#include <stddef.h>
#include <stdint.h>
#include <string.h>

#include "platform.h"

#include "util.h"
#include "net.h"

#define LOOPBACK_MTU UINT16_MAX /* maximum size of IP datagram */
#define LOOPBACK_QUEUE_LIMIT 16
#define LOOPBACK_IRQ (INTR_IRQ_BASE+1)

#define PRIV(x) ((struct loopback *)x->priv)

struct loopback { // loopback device内で使用するprivate dataを格納するための構造体
    int irq;
    mutex_t mutex;
    struct queue_head queue;
};

struct loopback_queue_entry { // queueのentryの構造体：data本体とmetadataを格納
    uint16_t type;
    size_t len;
    uint8_t data[]; // flexible array member：この構造体の最後のメンバ．構造体の最後にだけ配置できるサイズ不明の配列
};

static int // 渡されたデータをqueueに格納．割り込みを発生
loopback_transmit(struct net_device *dev, uint16_t type, const uint8_t *data, size_t len, const void *dst)
{
    struct loopback_queue_entry *entry;
    unsigned int num;

    mutex_lock(&PRIV(dev)->mutex); // queueへのアクセスをmutexで保護
    if (PRIV(dev)->queue.num >= LOOPBACK_QUEUE_LIMIT) { // queueの上限を超えていたらエラー
        mutex_unlock(&PRIV(dev)->mutex);
        errorf("queue if full");
        return -1;
    }
    entry = memory_alloc(sizeof(*entry) + len); // lenはデータ部分のサイズ
    if (!entry) {
        mutex_unlock(&PRIV(dev)->mutex);
        errorf("memory_alloc() failure");
        return -1;
    }
    entry->type = type;
    entry->len = len;
    memcpy(entry->data, data, len); // data本体のコピー
    queue_push(&PRIV(dev)->queue, entry); // entryをqueueへと格納
    num = PRIV(dev)->queue.num;
    mutex_unlock(&PRIV(dev)->mutex);
    debugf("queue pushed (num:%u), dev=%s, type=0x%04x, len=%zd", num, dev->name, type, len);
    debugdump(data, len);
    intr_raise_irq(PRIV(dev)->irq); // 割り込みを発生させる
    return 0;
}

static int // 割り込みハンドラ：queueからデータを取り出す．protocol stackの入力ハンドラを呼び出し
loopback_isr(unsigned int irq, void *id)
{
    struct net_device *dev;
    struct loopback_queue_entry *entry;

    dev = (struct net_device*)id;
    mutex_lock(&PRIV(dev)->mutex);
    while (1) {
        entry = queue_pop(&PRIV(dev)->queue); // queueからentryを取り出す
        if (!entry) {
            break; // 取り出すentryがなくなった
        }
        debugf("queue popped (num:%u), dev=%s, type=0x%04x, len=%zd", PRIV(dev)->queue.num, dev->name, entry->type, entry->len);
        debugdump(entry->data, entry->len);
        net_input_handler(entry->type, entry->data, entry->len, dev); // net_input_handlerに受信データ本体と付随する情報を渡す
        memory_free(entry);
    }
    mutex_unlock(&PRIV(dev)->mutex);
    return 0;
}

static struct net_device_ops loopback_ops = {
    .transmit = loopback_transmit,
};

struct net_device *
loopback_init(void) // デバイスの生成と登録．割り込みハンドラの登録
{
    struct net_device *dev;
    struct loopback *lo;

    dev = net_device_alloc();
    if (!dev) {
        errorf("net_device_alloc() failure");
        return NULL;
    }
    dev->type = NET_DEVICE_TYPE_LOOPBACK;
    dev->mtu = LOOPBACK_MTU;
    dev->hlen = 0;
    dev->alen = 0;
    dev->flags = NET_DEVICE_FLAG_LOOPBACK;
    dev->ops = &loopback_ops;

    lo = memory_alloc(sizeof(*lo));
    if (!lo) {
        errorf("memory_alloc() failure");
        return NULL;
    }
    lo->irq = LOOPBACK_IRQ;
    mutex_init(&lo->mutex);
    queue_init(&lo->queue);
    dev->priv = lo;

    if (net_device_register(dev) == -1) {
        errorf("net_device_register() failure");
        return NULL;
    }
    intr_request_irq(LOOPBACK_IRQ, loopback_isr, INTR_IRQ_SHARED, dev->name, dev);
    debugf("initialized device, dev=%s", dev->name);
    return dev;
}