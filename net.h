...
struct net_device {
    ...
    uint16_t mtu;
    uint16_t flags; // defined as NET_DEVICE_FLAG_XXX
    uint16_t hlen; /* header length */
    uint16_t alen; /* address length */
    uint8_t addr[NET_DEVICE_ADDR_LEN]; // hardeare address for device
    union {
        uint8_t peer[NET_DEVICE_ADDR_LEN];
        uint8_t broadcast[NET_DEVICE_ADDR_LEN];
    };
    struct net_device_ops *ops; // pointer to struct net_device_ops which is set functions implemented in device driver
